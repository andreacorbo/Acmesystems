...
# sudo apt -y install which sed make binutils build-essential diffutils gcc g++ bash patch gzip bzip2 perl tar cpio unzip rsync file bc findutils
...
BUILDROOT_TARGET=bertad2
BUILDROOT_DEFCONFIG=acme-bertad2_defconfig

BOOTSTRAP=at91bootstrap
BOOTSTRAP_TAG=4.0.4
BOOTSTRAP_TARGET=roadrunner
BOOTSTRAP_CONFIGS=configs/
BOOTSTRAP_DEFCONFIG=roadrunner_defconfig

KERNEL=linux-5.15-mchp
KERNEL_TARGET=acme-roadrunner
